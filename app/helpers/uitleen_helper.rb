...
module UitleenHelper
  def uitleen_work_url work
    if Rails.application.credentials.uitleen_site
      ...
      File.join(Rails.application.credentials.uitleen_site, "collections", collection_id.to_s, "works", work_id.to_s)
    end
  end
  ...
  def uitleen_new_draft_invoice_url params = {}
    if Rails.application.credentials.uitleen_site
      ...
      uri = URI.parse(Rails.application.credentials.uitleen_site)
