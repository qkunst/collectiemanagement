  ...
  class_methods do
    def preload_relations_for_display(display)
      ...
      when :limited, :limited_auction
        ...
        includes(:collection, :import_collection, :created_by, :library_items, :purchase_price_currency, :techniques, :object_categories, :time_spans, :damage_types, :frame_damage_types, :frame_type, :medium, :style, :themes, :subset, :sources, :owner, :work_status, :attachments, :appraisals, :condition_work, :condition_frame, :cluster, :placeability, :balance_category, :work_set_types, :geoname_summary, time_spans: [:contact], artists: [:artist_involvements], work_sets: [:time_spans, :work_set_type, :appraisals])
      end
    end
  end
end
