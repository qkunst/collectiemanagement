module ImportCollection::Json
  def write_json_work(work_data)
    as_is_fields = %w[id stock_number alt_number_1 alt_number_2 alt_number_3 alt_number_4 alt_number_5 alt_number_6 title title_unknown description object_creation_year object_creation_year_unknown print print_unknown frame_height frame_width frame_depth frame_diameter height width depth diameter public_description abstract_or_figurative location_detail location location_floor internal_comments inventoried refound new_found checked inventoried_at refound_at new_found_at checked_at geoname_id artist_unknown signature_comments no_signature_present information_back other_comments grade_within_collection medium_comments main_collection image_rights publish permanently_fixed condition_work_comments condition_frame_comments source_comments purchase_price purchased_on purchase_year selling_price minimum_bid market_value_max market_value_min replacement_value_min replacement_value_max valuation_on market_value replacement_value for_purchase for_rent highlight for_purchase_at for_rent_at highlight_priority work_data selling_price_minimum_bid_comments tag_list significantly_updated_at created_at removed_from_collection_at id]

    work = Work.new(work_data.slice(*as_is_fields))
  ...
  def import_time_spans work, work_data
    work_data["time_spans"]&.each do |time_span_data|
      ...
      contact.save
      time_span = TimeSpan.find_or_initialize_by(contact: contact, starts_at: time_span_data["starts_at"], subject: work, classification: time_span_data["classification"], collection: base_collection)
