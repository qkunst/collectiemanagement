...
class PdfPrinterWorker
  ...
  def perform(url, options = {})
    inform_user_id = options["inform_user_id"]
    subject_object_id = options["subject_object_id"]
    subject_object_type = options["subject_object_type"]
