...
class CollectionDownloadWorker
  ...
  def perform(collection_id, requested_by_user_id, format = :xlsx, audience = :default, fields_to_expose = [:id, :title_rendered])
    ...
    if format.to_sym == :xlsx
      ...
      Message.create(to_user_id: requested_by_user_id, from_user_name: "Download voorbereider", attachment: File.open(filename), message: "De download is gereed, open het bericht in je browser om de bijlage te downloaden.", subject: "Download #{collection.name} gereed")
    end
  end
end
