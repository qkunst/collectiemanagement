...
class Batch::WorkForm < Work
  BATCH_FIELDS = %w[purchase_price purchased_on purchase_year source_comments other_comments selling_price publish_selling_price minimum_bid location location_floor location_detail main_location_id cluster_id cluster_name subset_id technique_ids source_ids object_category_ids collection_id placeability_id logistical_peculiarity_ids theme_ids grade_within_collection tag_list work_status_id balance_category_id for_rent for_purchase highlight publish].sort_by(&:length).reverse.map(&:to_sym)
  ...
  REMOVABLE_FIELDS = %w[tag_list theme_ids technique_ids object_category_ids logistical_peculiarity_ids].map(&:to_sym)
