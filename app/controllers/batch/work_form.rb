...
class Batch::WorkForm < Work
  BATCH_FIELDS = %w[purchase_price purchased_on purchase_year source_comments other_comments selling_price publish_selling_price minimum_bid location location_floor location_detail main_location_id cluster_id cluster_name subset_id technique_ids source_ids collection_id placeability_id theme_ids grade_within_collection tag_list work_status_id balance_category_id for_rent for_purchase highlight publish].sort_by(&:length).reverse.map(&:to_sym)
