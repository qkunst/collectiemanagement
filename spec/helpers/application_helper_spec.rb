...
RSpec.describe ApplicationHelper, type: :helper do
  describe "#menu_link_to" do
    it "returns takes a description and task" do
      ...
      expect(helper.menu_link_to("description", "books/1")).to eq("<a href=\"books/1\">description</a>")
    end
    it "should mark equal path active" do
      allow(helper.request).to receive(:path).and_return "books/1"
      expect(helper.menu_link_to("description", "books/1")).to eq("<a class=\"active\" aria-current=\"page\" href=\"books/1\">description</a>")
    end
    it "should mark equal path active also with :only_exact_path_match-option true" do
      allow(helper.request).to receive(:path).and_return "books/1"
      expect(helper.menu_link_to("description", "books/1", {only_exact_path_match: true})).to eq("<a class=\"active\" aria-current=\"page\" href=\"books/1\">description</a>")
    end
    ...
    it "should not mark child path active if only_exact_path_match: true" do
      allow(helper.request).to receive(:path).and_return "books/1/page/1"
      expect(helper.menu_link_to("description", "books/1", {only_exact_path_match: true})).to eq("<a href=\"books/1\">description</a>")
    end
    it "should work with full urls as well" do
      allow(helper.request).to receive(:path).and_return "books/1/page/1"
      expect(helper.menu_link_to("description", "http://example.com/books/1")).to eq("<a class=\"active\" aria-current=\"page\" href=\"http://example.com/books/1\">description</a>")
    end
  end
